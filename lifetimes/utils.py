...
def calculate_alive_path(
    ...
    customer_history["recency"] = customer_history["recency"].ffill().fillna(0)
...
def _customer_lifetime_value(
    ...
    # Convert to numpy arrays to avoid index alignment issues and ensure vectorized math
    frequency = np.asarray(frequency)
    recency = np.asarray(recency)
    T = np.asarray(T)
    monetary_value = np.asarray(monetary_value)

    # initialize the clv vector to zeros
    clv = np.zeros(len(frequency), dtype=float)
    ...
    for i in steps * factor:
        ...
        exp_curr = np.asarray(transaction_prediction_model.predict(i, frequency, recency, T))
        exp_prev = np.asarray(transaction_prediction_model.predict(i - factor, frequency, recency, T))
        expected_number_of_transactions = exp_curr - exp_prev
        # sum up the CLV estimates of all of the periods and apply discounted cash flow
        clv += (monetary_value * expected_number_of_transactions) / (1 + discount_rate) ** (i / factor)

    return pd.Series(clv) # return as a series
