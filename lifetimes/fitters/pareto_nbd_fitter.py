...
class ParetoNBDFitter(BaseFitter):
    ...
    def _log_A_0(
        ...
        # Sanitize potential NaNs from hypergeometric evaluation
        p_1 = np.nan_to_num(p_1, nan=0.0)
        p_2 = np.nan_to_num(p_2, nan=0.0)

        # Use absolute values inside logs and carry signs separately to avoid NaNs
        sign1 = np.sign(p_1)
        sign2 = np.sign(p_2)
        v1 = log(np.abs(p_1)) + rsf * log(q_2)
        v2 = log(np.abs(p_2)) + rsf * log(q_1)
        ls, _ = logsumexp([v1, v2], b=[sign1, -sign2], axis=0, return_sign=True)
        res = ls - rsf * log(q_1 * q_2)
        # If recency == age, the two terms cancel exactly; return log(0) = -inf for those entries
        eq = (recency == age)
        if np.isscalar(eq):
            if eq:
                return -np.inf
