...
class TestBetaGeoBetaBinomFitter:
    ...
    def test_prob_alive_is_close_to_Hardie_paper_table_6(self, donations):
        ...
        prob_list[0] = (
            bbtf.data[(bbtf.data["frequency"] == 0) & (bbtf.data["recency"] == 0)]["prob_alive"].iloc[0]
        )
        prob_list[1] = (
            bbtf.data[(bbtf.data["frequency"] == 2) & (bbtf.data["recency"] == 4)]["prob_alive"].iloc[0]
        )
        prob_list[2] = (
            bbtf.data[(bbtf.data["frequency"] == 6) & (bbtf.data["recency"] == 6)]["prob_alive"].iloc[0]
        )
    ...
    def test_fit_with_and_without_weights(self, donations):
        ...
        exploded_dataset = pd.concat(frames, ignore_index=True)
...
class TestModifiedBetaGammaFitter:
    ...
    def test_params_out_is_close_to_BTYDplus(self, cdnow_customers):
        """Reference: BTYDplus (R implementation)."""
